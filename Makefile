...
$(subst .tex,.pdf,$(manuscript)): $(manuscript) $(includes) $(figures) $(subst .tex,.bbl,$(manuscript))
	pdflatex $<
	pdflatex $<
...
$(patsubst %.tex,%.aux,$(manuscript) $(includes)): $(manuscript) $(includes)
	latex $<
...
AutoBibliography.bib: $(manuscript) $(includes) fetch_bibliography.py
	pdflatex $(manuscript)
	python fetch_bibliography.py -filename $(manuscript)
#	cat $(filter %.aux,$(patsubst %.tex,%.aux,$<)) >$(subst .bib,.aux,$@)
#	ln -sf $< $(subst .bib,.tex,$@)
#	./makebib $(subst .bib,,$@)

$(tar): $(manuscript) $(includes) $(figures) $(bibliography) Makefile fetch_bibliography.py
	tar czvfh $@ $^
...
test:
	@echo $(manuscript) $(includes) $(figures)
