...
While we can roughly estimate that CO WDs have a range of masses from $\sim0.4$ and $\sim1.0\,\Msun$ (Sec. \ref{ssec:c1_ddwdform}), what are the typical masses involved in a CO WD merger?

\begin{itemize}
	\item Differences in DD systems between population synthesis codes are due to differences between the implementations of mass loss and WD formation for single stars, the details of mass transfer and criteria for when it is stable, and treatment of the common envelope phase \citep{toon+14}.
\end{itemize}
